...
exports.manageState = async (event) => {
  ...
  if ('sendOrderToRestaurant' in event) {
    ...
    const result = updatePut(params, orderStatus, event);
    return result;
  }
...
const updatePut = async (params, orderStatus, event) => {
  try {
  ...
  await snsPublish(orderStatus, event);
...
const snsPublish = async ( status, event) =>{
  let message = `Order Status: ${status} for ${event.name} x${event.quantity}`;
  try {
    const input = {
